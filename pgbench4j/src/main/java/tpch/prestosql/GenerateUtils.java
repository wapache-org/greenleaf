...
public final class GenerateUtils
{
    ...
    /**
     *
     * @param scaleBase 基准数量
     * @param scaleFactor 比例因子, 总数据量等于 scaleBase * scaleFactor
     * @param part 第几页
     * @param partCount 分成多少页
     * @return 第part页第一行的偏移量.
     */
    ...
    public static int findDate(String date){
        for (int i = 0; i < DATE_STRING_INDEX.size(); i++) {
            if(DATE_STRING_INDEX.get(i).equals(date)){
                return i;
            }
        }
        return -1;
