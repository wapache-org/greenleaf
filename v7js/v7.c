...
/*
 * Matches 0-terminated string (v7_match_prefix) or string with given length
 * v7_match_prefix_n against a glob pattern.
 *
 * Match is case-insensitive. Returns number of bytes matched, or -1 if no
 * match.
 */
int v7_match_prefix(const char *pattern, int pattern_len, const char *str);
int v7_match_prefix_n(const struct mg_str pattern, const struct mg_str str);
...
int v7_match_prefix_n(const struct mg_str, const struct mg_str) WEAK;
int v7_match_prefix_n(const struct mg_str pattern, const struct mg_str str) {
  const char *or_str;
  size_t len, i = 0, j = 0;
  int res;
  ...
  if ((or_str = (const char *) memchr(pattern.p, '|', pattern.len)) != NULL ||
    ...
    res = v7_match_prefix_n(pstr, str);
    if (res > 0) return res;
    pstr.p = or_str + 1;
    pstr.len = (pattern.p + pattern.len) - (or_str + 1);
    return v7_match_prefix_n(pstr, str);
  }
  ...
  for (; i < pattern.len; i++, j++) {
    if (pattern.p[i] == '?' && j != str.len) {
      ...
      do {
        ...
        res = v7_match_prefix_n(pstr, sstr);
      } while (res == -1 && len-- > 0);
...
int v7_match_prefix(const char *, int, const char *) WEAK;
int v7_match_prefix(const char *pattern, int pattern_len, const char *str) {
  const struct mg_str pstr = {pattern, (size_t) pattern_len};
  struct mg_str s = {str, 0};
  if (str != NULL) s.len = strlen(str);
  return v7_match_prefix_n(pstr, s);
}
...
enum v7_err v7_mk_regexp(struct v7 *v7, const char *re, size_t re_len,
  ...
  if (slre_compile(re, re_len, flags, flags_len, &p, 1) != SLRE_OK || p == NULL) {
    printf("v7_mk_regexp: len=%d, str=%s\n", re_len, re);
